...
# def bending_energy_Willmore(vertex_Normal): 
#     '''
#     Computes the bending energy of the cloth for the vertex positions v
#     Reference: ArcSim (physics.cpp)
#     '''
...
def bending_energy_Willmore(v, cloth, vertex_Normal , return_average=True): 
    '''
    Computes the bending energy of the cloth for the vertex positions v
    Reference: ArcSim (physics.cpp)
    '''
    ...
    # Formula:
    # \sum_{triangles} {Area in uv space}\sqrt{\det G} (\tr{-F^T F_n G^-1})^2
    # = \sum_{triangles} {Area in R^3} (\tr{-F^T F_n G^-1})^2

    epsilon = 1e-7
    batch_size = tf.cast(tf.shape(v)[0], v.dtype)
    triangles = gather_triangles(v, cloth.f)
    triangles_Normal = gather_triangles(vertex_Normal, cloth.f)    #return vertex 3 tuples
    ...
    # print("----------------------")
    # print(cloth)
    # print(Dm_inv.shape, Ds.shape) #(2, 8710, 2, 2) (2, 8710, 3, 2)
    F = Ds @ Dm_inv 
    
    Ft = tf.transpose(F, perm=[0, 1, 3, 2])
    # print(F.shape, Ft.shape) #(2, 8710, 3, 2) (2, 8710, 2, 3)
    G = Ft @ F
    try:
    ...
    # print(G.shape, G_inv.shape) #(2, 8710, 2, 2) (2, 8710, 2, 2)
    Ds_2 = get_shape_matrix(triangles_Normal)
    F_n = Ds_2 @ Dm_inv

    tmp = tf.linalg.trace(Ft @ F_n @ G_inv) 
    # print((Ft @ F_n @ G_inv).shape) #(2, 8710, 2, 2)
    # print(tmp.shape, (tmp*tmp).shape) #(2, 8710) (2, 8710)
    #compute Area of triangeles in R^3
    # Compute face normals
    fn = FaceNormals(dtype=v.dtype, normalize=False)(v, cloth.f)
     
    # Compute the ultimate Willmore Functional

    energy = tf.math.reduce_euclidean_norm(fn, axis=-1) * tmp * tmp
    # print(fn)
    # print(tf.math.reduce_euclidean_norm(fn, axis=-1)) #(2,8710)
    # print(tmp)
    # print(energy)
...
def inertial_term(x, x_prev, v_prev, mass, time_step, return_average=True):
    batch_size = tf.cast(tf.shape(x)[0], x.dtype) - 1 #第一个不能算
...
def inertial_term_sequence(x, mass, time_step, return_average=True):
    ...
    # Compute velocities
    x_current = x[:, 1:]
    # print(x_current)# shape=(1, 2, 4424, 3) 去掉第一个
    x_prev = x[:, :-1] 
...
if __name__ == "__main__":
    ...
    print(dummy_input.shape, dummy_input_flat.shape)
    #(128, 3, 4424, 3) (384, 4424, 3)
