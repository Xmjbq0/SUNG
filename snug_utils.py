...
def load_motion(path):
    ...
    #print("")
    # print("load [MOTION]", motion)

    # lst = motion.files
    # for item in lst:
    #     print(item)
    #     print(motion[item].shape)
    '''
    trans
    (2751, 3)
    gender
    ...
    (2751, 156)
    shape parameters,
    β, the pose parameters, 
    θ, and translation of the root in the
    world coordinate system, γ.
    For simplicity of notation, we include the 3D translation vector
    γ in the pose vector.
    The pose θ is determined by a pose
    vector of 3 × 52 + 3 = 159 parameters.
    which adds hand articulation to the model
    using a total of n = 52 joints, where 22 joints are for the
    body and the remaining 30 joints belong to the hands.
    '''
    ...
    #print(pose.shape, trans.shape, trans_vel.shape)
    #(688, 72) (688, 3) (688, 3)
    return pose.astype(np.float32), trans.astype(np.float32), trans_vel.astype(np.float32)

def load_motion_train_With_betas(path):
    motion = np.load(path, mmap_mode='r')
    ...
    #print(pose.shape, trans.shape, trans_vel.shape)
    #(688, 72) (688, 3) (688, 3)
    return pose.astype(np.float32), trans.astype(np.float32), trans_vel.astype(np.float32), motion['betas'][:10]
